...
// Determines if a character is punctuation as defined by the quarkdown spec.
...
static bool parse_qcmd(Scanner *s, TSLexer *lexer,
    ...
    if (valid_symbols[QCMD_MARKER] && s->indentation == 0) {
        ...
        if (
            ...
            lexer->result_symbol = QCMD_MARKER;
            mark_end(s, lexer);
            return true;
        }
    }
...
static bool scan(Scanner *s, TSLexer *lexer, const bool *valid_symbols) {
    ...
    if (!(s->state & STATE_MATCHING)) {
        ...
        switch (lexer->lookahead) {
            ...
            case '.':
                // A '.' could mark a qcmd
                return parse_qcmd(s, lexer, valid_symbols);
            case '=':
...
void *tree_sitter_quarkdown_external_scanner_create(void) {
...
bool tree_sitter_quarkdown_external_scanner_scan(void *payload, TSLexer *lexer,
                                                const bool *valid_symbols) {
...
unsigned tree_sitter_quarkdown_external_scanner_serialize(void *payload,
                                                         char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize(scanner, buffer);
}

void tree_sitter_quarkdown_external_scanner_deserialize(void *payload,
                                                       const char *buffer,
                                                       unsigned length) {
    Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_quarkdown_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
